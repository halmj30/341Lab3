    ...
    public class RelationalDatabase : IDatabase
    {
        ...
        public void AddEntry(Entry entry)
        {
            try
            {
                ...
                //INSERT INTO Lab3 VALUES (1, 'Rouge One', 'The best starwars', 0, 10/09/2022);

                using var con = new NpgsqlConnection(connectionString);
                con.Open();

                var command = $"INSERT INTO Lab3 VALUES ({entry.id}, {entry.answer}, {entry.clue}, {entry.difficulty}, {entry.date})";
        ...
        public bool DeleteEntry(Entry entry)
        {
            try
            {
                ...
                var command = "DELETE FROM Lab3 WHERE id = entry.id";
                SQLCommand sqlcommand = new SQLCommand(command, con);
                sqlcommand.ExecuteNonQuery();
            ...
            foreach (Entry entry in entries) // iterate through entries until we find the Entry in question
            {
                if (entry.Id == replacementEntry.Id) // found it
                {
                    ...
                    try
                    {
                        ...
                        var command = $"UPDATE Lab3 SET " +
                            $"@id = {entry.id}, @answer = {entry.answer}, @clue = {entry.clue}, " +
        ...
        public String InitializeConnectionString()
        {
            ...
            var bitApiKey = "v2_3uhXm_ujdWK2uTSRw8dygR7iQAdgG"; // from the "Password" field of the "Connect" menu
